...
class ForumSubscriptionForm(forms.Form):
    def __init__(self, user, forum, instance=None, *args, **kwargs):
        ...
        if instance :
            type_choices.append(
                ('unsubscribe', _('Usunąć subskrypcje z tego forum.')))
            type_initial = instance.type
        else:
            type_initial = ForumSubscription.TYPE_NOTIFY
        self.fields['type'] = forms.ChoiceField(
            label=_('Chcę:'), choices=type_choices, initial=type_initial,
            widget=forms.RadioSelect())

        topic_choices = (
            ('new', _('Nowych tematach.')),
            ('all', _('Wszystkich tematach na tym forum.')),
        )
        self.fields['topics'] = forms.ChoiceField(
            label=_('Będę otrzymywać powiadomienia o:'), choices=topic_choices,
