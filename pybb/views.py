...
class ForumSubscriptionView(RedirectToLoginMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        if result == 'subscribe-all':
            msg = _((
                'Zasubskrybowałeś wszystkie istniejące tematy na tym forum '
                'oraz wszystkie przyszłe tematy będą subskrybowane automatycznie'
            ))
        elif result == 'delete':
            msg = _((
                'Nie będziesz więcej informowany o każdym nowym temacie '
                'założonym na tym forum.'
            ))
        elif result == 'delete-all':
            msg = _((
                'Zasubskrybowałeś każdy obecny temat na tym forum, ale nie będziesz '
                'otrzymywał powiadomień o nowo utworzonych tematach.'
            ))
        else:
            msg = _((
                'Będziesz powiadamiany za każdym razem kiedy nowy temat zostanie utworzony na tym forum.'
            ))
...
class TopicView(RedirectToLoginMixin, PaginatorMixin, PybbFormsMixin, generic.ListView):
    ...
    def get_topic(self, **kwargs):
        if 'pk' in kwargs:
            ...
            raise Http404(_('Ten temat nie istnieje.'))
        return topic
...
def delete_subscription(request, topic_id):
    ...
    msg = _('Subskrypcja usunięta. Nie będziesz otrzymywał wiadomości email o nowej aktywności chyba, że zasubskrybujesz lub będziesz aktywny w tym temacie.')
...
def add_subscription(request, topic_id):
    ...
    msg = _('Subskrybcja dodana. Będziesz otrzymywał powiadomienia w formie wiadomości email za każdym razem kiedy w tym temacie pojawi się nowa aktywność.')
    messages.success(request, msg, fail_silently=True)
    return HttpResponseRedirect(topic.get_absolute_url())
...
def mark_all_as_read(request):
    ...
    msg = _('Wszystkie fora oznaczone jako przeczytane.')
    messages.success(request, msg, fail_silently=True)
...
def block_user(request, username):
    ...
    msg = _('Użytkownik zablokowany pomyślnie.')
    messages.success(request, msg, fail_silently=True)
...
def unblock_user(request, username):
    ...
    user.is_active = True
    user.save()
    msg = _('Użytkownik odblokowany pomyślnie.')
    messages.success(request, msg, fail_silently=True)
    return redirect('pybb:index')
...
class UserEditPrivilegesView(generic.edit.FormMixin, generic.edit.ProcessFormView, generic.DetailView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _("Prawa użytkownika zostały zaktualizowane."))
